...
const API_URL = "http://192.168.18.203:5000/api";
...
export const sendPhoneVerification = async (phone) => {
  ...
  const response = await fetch(`${API_URL}/verification/send-phone`, {
    method: "POST",
    headers: { 
...
export const verifyPhone = async (code) => {
  const token = await getToken();
  const response = await fetch(`${API_URL}/verification/verify-phone`, {
    method: "POST",
    headers: { 
      "Content-Type": "application/json",
      "Authorization": `Bearer ${token}`
...
export const resendPhoneVerification = async (phone) => {
  const token = await getToken();
  const response = await fetch(`${API_URL}/verification/resend-phone`, {
    method: "POST",
    headers: { 
      "Content-Type": "application/json",
      "Authorization": `Bearer ${token}`
